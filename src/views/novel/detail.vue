<template>
  <div class="page-shell sub-page comic-detail">
        ...
        <div v-if="item" class="hero-main">
          ...
          <div class="meta">
            ...
            <p class="sub">
              ...
              <span class="chap-num">共{{ item.chapter_count || 0 }}章</span>
            </p>
            <p class="stats">
    ...
    <div class="hero-extra">
      ...
      <p v-if="item?.intro" class="intro"><b>简介：</b>{{ item.intro }}</p>
    ...
    <div class="panel">
      ...
      <div v-if="tab === 'detail'" class="scroll-body">
        <section class="catalog">
          ...
          >
            ...
            <span class="ch-name">{{ chapterName(ch) }}</span>
            <span v-if="ch.playable" class="watch">阅读</span>
      ...
      <CommentPanel
        ...
        :content-id="item.id"
        :media-type="COMMENT_MEDIA_NOVEL"
...
import { fetchCollectList, MEDIA_NOVEL, operateCollect } from '@/api/collect'
import {
  buyNovel,
  chapterName,
  fetchNovelChapters,
...
import { COMMENT_MEDIA_NOVEL, fetchComments } from '@/api/ops'
...
const item = ref<NovelDetail | null>(null)
...
const categories = computed(() => novelCategories(item.value || {}))
...
const loadFav = async () => {
  ...
  try {
    const data = await fetchCollectList(1, MEDIA_NOVEL, 1, 100)
...
const load = async () => {
  ...
  const [d, c] = await Promise.all([fetchNovelDetail(id), fetchNovelChapters(id)])
  item.value = d
  ...
  pushBrowse({
    ...
    sub: d.chapter_count ? `共${d.chapter_count}章` : '',
  })
...
const loadCommentCount = async () => {
  ...
  if (!id) return
  try {
    const data = await fetchComments(id, 1, 1, 0, COMMENT_MEDIA_NOVEL)
...
const buy = async () => {
  ...
  try {
    const res = await buyNovel(item.value.id)
...
const openChapter = (ch: NovelChapter) => {
  ...
  router.push(novelReadPath(item.value?.id || routeId(route.params.id), ch.id))
}
...
const back = async () => {
  ...
  const prevIsDup = Boolean(prev) && (prev === here || (enc && prev.startsWith(`/novel/${enc}/read/`)))
  if (prevIsDup) {
    await router.go(-2)
    const now = router.currentRoute.value
    if (now.fullPath.split('?')[0] === here || now.name === 'NovelRead') {
...
const onFav = async () => {
  ...
  try {
    await operateCollect(item.value.id, MEDIA_NOVEL, next)
