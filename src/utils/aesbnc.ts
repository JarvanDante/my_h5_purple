/** 对齐公司 AesUtil::decryptRaw: AES-128-ECB + PKCS7, key 为 16 字节 ASCII。 */
const KEY_STR = '525202f9149e061d'
...
const roundKey = expandKey(new TextEncoder().encode(KEY_STR))

function addRoundKey(s: Uint8Array, off: number) {
  for (let i = 0; i < 16; i++) s[i] ^= roundKey[off + i]
}
